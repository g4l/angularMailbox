...
MailApp.service('MyMails', function($http) {
    ...
    this.getAllLetters = () => {
      return $http.get('http://test-api.javascript.ru/v1/avoznuk2/letters')
        .then(response => response.data)
    }
    this.getMailBoxes = () => {
      return $http.get('http://test-api.javascript.ru/v1/avoznuk2/mailboxes')
        .then(response => response.data)
    }

    this.add = (mail) => {
      return $http.post('http://test-api.javascript.ru/v1/avoznuk2/letters', mail)
        .then(response => response.data)
    }
  })
  .component('mailbox', {
